...
SNOWFLAKE_WAREHOUSE = os.getenv("SNOWFLAKE_WARE")
...
def init_snowflake_connection():
    try:
        ...
        logger.error(f"Error initializing Snowflake connection: {e}")
        raise 
...
def create_snowflake_table(conn):
    ...
    try:
        ...
        conn.commit()
        logger.info("Snowflake table created successfully")
    except Exception as e:
...
def read_processed_data(s3_client, execution_date): 
    logger.info("\n --------Reading processed data from S3--------\n")       
    
    s3_prefix = f"processed/historical/date={execution_date}"
    
    logger.info(f"Reading data from S3 prefix: {s3_prefix}")
    
    try:
        response = s3_client.list_obejcts_v2(Bucket = MINIO_BUCKET, Prefix = s3_prefix)
        ...
        for obj in response["Contents"]:
            if obj['Key'].endswith('.parquet'):
                ...
                try:
                    ...
                    if symbol is None:
                        logger.error(f"Failed to extract symbol from file path: {obj['Key']}")
                        continue
                        
                    response = s3_client.get_object(Bucket=MINIO_BUCKET, Key=obj['Key'])
                    ...
                    continue
        if not dfs:
            logger.info(f"No data found for date: {execution_date}")
            return None    
        
        processed_df = pd.concat(dfs, ignore_index=True)
        logger.info(f"Read {len(processed_df)} rows from MinIO for date")
        
        if "date" in processed_df.columns:
            processed_df["date"] = pd.to_datetime(processed_df["date"]).dt.date
        else:
            logger.error("Missing 'date' column in the dataframe")
        ...
        logger.error(f"Failed to list objects in bucket {MINIO_BUCKET}:")
        return None
                    
                    
def incremental_load_to_snowflake(conn, df):
    logger.info("\n ------ Performing incremental load to Snowflake ------ \"")
    
    if df is None or df.empty:
        logger.info("No data to load into")
        return
    ...
    try:
        ...
        for record in records:
            record_tuple = tuple(None if pd.isna(val) else
...
def main():
    ...
    try:
        ...
        if df is not None:
            ...
            logger.info(f"No new data to load for {execution_date}")
        
    except Exception as e:
        logger.exception("Failed to execute commands on Snowflake: {e}")
        sys.exit(1)
    finally:
        conn.close()
